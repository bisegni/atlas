#!/usr/bin/env bash
# Exact mixed-Q4/Q6 FFN-down layout A/B: --screen uses two runs.
set -euo pipefail

repo_root=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
cd "$repo_root"

baseline_env=ATLAS_GEMMA4_FFN_DOWN_EXPERIMENT=baseline
candidate_env=ATLAS_GEMMA4_FFN_DOWN_EXPERIMENT=interleaved16
runs=5
promotion=true
while (($#)); do
    case "$1" in
        ...
        jq -s --arg workload "$workload" --argjson runs "$runs" '
          ...
          . as $records | {
            ...
            checks: {
              ...
              selected_ffn_down_kernel: all($records[]; .selected_kernels.ffn_down_projection != "none")
            }
...
jq -n --arg baseline_env "$baseline_env" --arg candidate_env "$candidate_env" --argjson promotion "$promotion" \
  ...
  --slurpfile candidate_short "$artifact_dir/candidate/short-summary.json" '
    ...
    | {baseline_environment: $baseline_env, candidate_environment: $candidate_env, baseline: {long: $base_long, short: $base_short}, candidate: {long: $candidate_long, short: $candidate_short}, comparison: {
        ...
        stable_weight_upload_accounting: ($base_long.records[0].weight_upload_bytes == $candidate_long.records[0].weight_upload_bytes and $base_short.records[0].weight_upload_bytes == $candidate_short.records[0].weight_upload_bytes),
        ...
        baseline_pipeline_selected: ($base_long.records[0].selected_kernels.ffn_down_projection == "matvec_q4_0_16row" and $base_short.records[0].selected_kernels.ffn_down_projection == "matvec_q4_0_16row"),
        candidate_pipeline_selected: ($candidate_long.records[0].selected_kernels.ffn_down_projection == "matvec_q4_0_16row_ffn_down_interleaved" and $candidate_short.records[0].selected_kernels.ffn_down_projection == "matvec_q4_0_16row_ffn_down_interleaved"),
