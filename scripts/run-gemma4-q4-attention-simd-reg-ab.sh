#!/usr/bin/env bash
# Screen the register-resident one-barrier packed-Q4 two-pass attention
# candidate. This never promotes the opt-in selector; it records exact parity
# and matched throughput.
set -euo pipefail
...
artifact_dir="artifacts/phase-12.3-q4-attention-simd-reg-ab/${stamp}"
...
run_mode candidate 2pass_simd_reg attention_decode_fused_gemma4_simd_q4_0_2pass_simd_reg
...
jq -n \
    ...
    {
      purpose: "screen-only register-resident one-barrier packed-Q4 two-pass attention experiment; production default unchanged",
      model_id: $model_id,
      fixture: $fixture,
      ...
      environment: {
        ...
        candidate_attention: "2pass_simd_reg"
      },
...
' > "$artifact_dir/q4-attention-simd-reg-ab-summary.json"

summary="$artifact_dir/q4-attention-simd-reg-ab-summary.json"
jq '{pass, comparison}' "$summary"
echo "Q4 attention simd-reg screen: $summary"
if jq -e '.pass == true' "$summary" >/dev/null; then
    echo "Q4 ATTENTION SIMD-REG: SCREEN COMPLETE (no promotion)"
else
    echo "Q4 ATTENTION SIMD-REG: SCREEN FAILED" >&2
    exit 1
fi
