#!/usr/bin/env bash
# Screen the one-SIMD-group, barrier-free two-pass attention candidate. This
# never promotes the opt-in selector; it records exact parity and matched
# throughput. It is the evidence gate for per-key threadgroup barrier removal
# before the register-resident simd_reg variant is written.
set -euo pipefail
...
artifact_dir="artifacts/phase-12.3-q4-attention-simd-ab/${stamp}"
...
run_mode candidate 2pass_simd attention_decode_fused_gemma4_simd_q4_0_2pass_1_simd
...
jq -n \
    ...
    {
      purpose: "screen-only one-SIMD-group barrier-free two-pass attention experiment; production default unchanged",
      model_id: $model_id,
      fixture: $fixture,
      ...
      environment: {
        ...
        candidate_attention: "2pass_simd"
      },
...
' > "$artifact_dir/q4-attention-simd-ab-summary.json"

summary="$artifact_dir/q4-attention-simd-ab-summary.json"
jq '{pass, comparison}' "$summary"
echo "Q4 attention simd screen: $summary"
if jq -e '.pass == true' "$summary" >/dev/null; then
    echo "Q4 ATTENTION SIMD: SCREEN COMPLETE (no promotion)"
else
    echo "Q4 ATTENTION SIMD: SCREEN FAILED" >&2
    exit 1
fi
