#!/usr/bin/env bash
# Exact mixed-Q4/Q6 attention-unroll A/B: --screen uses two runs.
set -euo pipefail
...
candidate_env=ATLAS_GEMMA4_Q4_ATTENTION_EXPERIMENT=2pass_unroll2_no_value_barrier
runs=5
promotion=true
while (($#)); do
    case "$1" in
...
artifact_dir="artifacts/phase-12a-q4-attention-unroll-ab/${stamp}"
...
jq -n --arg baseline_env "$baseline_env" --arg candidate_env "$candidate_env" --argjson promotion "$promotion" \
  ...
  --slurpfile candidate_short "$artifact_dir/candidate/short-summary.json" '
    ...
    | {baseline_environment: $baseline_env, candidate_environment: $candidate_env, baseline: {long: $base_long, short: $base_short}, candidate: {long: $candidate_long, short: $candidate_short}, comparison: {
        ...
        baseline_pipeline_selected: ($base_long.records[0].selected_kernels.attention == "attention_decode_fused_gemma4_simd_q4_0_2pass_no_value_barrier" and $base_short.records[0].selected_kernels.attention == "attention_decode_fused_gemma4_simd_q4_0_2pass_no_value_barrier"),
        candidate_pipeline_selected: ($candidate_long.records[0].selected_kernels.attention == "attention_decode_fused_gemma4_simd_q4_0_2pass_unroll2_no_value_barrier" and $candidate_short.records[0].selected_kernels.attention == "attention_decode_fused_gemma4_simd_q4_0_2pass_unroll2_no_value_barrier"),
  ...
  ' | tee "$artifact_dir/q4-attention-unroll-ab-summary.json"

echo "Q4 attention unroll A/B: $artifact_dir/q4-attention-unroll-ab-summary.json"
if jq -e '.pass == true' "$artifact_dir/q4-attention-unroll-ab-summary.json" >/dev/null; then
    if [[ "$promotion" == true ]]; then
        echo "Q4 ATTENTION UNROLL A/B: PASS"
    else
        echo "Q4 ATTENTION UNROLL SCREEN: PASS (not eligible for promotion)"
    fi
else
    echo "Q4 ATTENTION UNROLL A/B: NO PROMOTION" >&2
    exit 1
fi
