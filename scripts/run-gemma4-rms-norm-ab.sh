#!/usr/bin/env bash
# Exact mixed-Q4/Q6 RMS decode rollback A/B: --screen uses two runs.
set -euo pipefail
        ...
        jq -s --arg workload "$workload" --argjson runs "$runs" '
          ...
          . as $records | {
            ...
            checks: {
              ...
              selected_rms_kernel: all($records[]; .selected_kernels.rms_norm != "none")
            }
...
jq -n --arg baseline_env "$baseline_env" --argjson promotion "$promotion" \
  ...
  --slurpfile candidate_short "$artifact_dir/candidate/short-summary.json" '
    ...
    | {baseline_environment: $baseline_env, candidate_environment: "-", baseline: {long: $base_long, short: $base_short}, candidate: {long: $candidate_long, short: $candidate_short}, comparison: {
        ...
        baseline_pipeline_selected: ($base_long.records[0].selected_kernels.rms_norm == "rms_norm_decode_f32" and $base_short.records[0].selected_kernels.rms_norm == "rms_norm_decode_f32"),
        candidate_pipeline_selected: ($candidate_long.records[0].selected_kernels.rms_norm == "rms_norm_decode_f32_vec4" and $candidate_short.records[0].selected_kernels.rms_norm == "rms_norm_decode_f32_vec4"),
  ...
  ' | tee "$artifact_dir/rms-norm-ab-summary.json"

echo "RMS norm A/B: $artifact_dir/rms-norm-ab-summary.json"
if jq -e '.pass == true' "$artifact_dir/rms-norm-ab-summary.json" >/dev/null; then
    if [[ "$promotion" == true ]]; then
        echo "RMS NORM A/B: PASS"
    else
        echo "RMS NORM SCREEN: PASS (not eligible for promotion)"
    fi
else
    echo "RMS NORM A/B: NO PROMOTION" >&2
    exit 1
fi
