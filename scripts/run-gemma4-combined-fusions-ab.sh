#!/usr/bin/env bash
# Combined exact dispatch-fusion candidate A/B for Phase 1:
#   ATLAS_GEMMA4_QK_NORM_ROPE_EXPERIMENT=fused
#   ATLAS_GEMMA4_FFN_GELU_MULTIPLY_EXPERIMENT=fused
#   ATLAS_GEMMA4_PLE_COMPOSITION_EXPERIMENT=fused
# --screen uses two runs; normal uses five.
set -euo pipefail
...
baseline_env=""
candidate_env="ATLAS_GEMMA4_QK_NORM_ROPE_EXPERIMENT=fused ATLAS_GEMMA4_FFN_GELU_MULTIPLY_EXPERIMENT=fused ATLAS_GEMMA4_PLE_COMPOSITION_EXPERIMENT=fused"
runs=5
promotion=true
while (($#)); do
    case "$1" in
...
artifact_dir="artifacts/phase-12.3-combined-fusions-ab/${stamp}"
...
clean_env=(
    ...
    -u ATLAS_GEMMA4_QKV_FUSED_EXPERIMENT
...
jq -n --arg baseline_env "$baseline_env" --arg candidate_env "$candidate_env" --argjson promotion "$promotion" \
  ...
  | .pass = (all(.baseline[]; .pass) and all(.candidate[]; .pass) and all(.comparison["exact_long","exact_short","selected_formats_identical","stable_long_accounting","stable_short_accounting","dispatches_reduced","short_not_regressed"]; . == true) and (if .comparison.promotion_eligible then .comparison.long_improved else (.comparison.long_speedup_percent >= 0) end))
  ' | tee "$artifact_dir/combined-fusions-ab-summary.json"

echo "Combined fusions A/B: $artifact_dir/combined-fusions-ab-summary.json"
jq -e '.pass == true' "$artifact_dir/combined-fusions-ab-summary.json" >/dev/null || { echo "COMBINED FUSIONS: NO PROMOTION" >&2; exit 1; }
[[ "$promotion" == true ]] && echo "COMBINED FUSIONS: PASS" || echo "COMBINED FUSIONS SCREEN: PASS"
