#!/usr/bin/env bash
# Decode-only QK norm + RoPE fusion A/B (7 dispatches/layer -> 1).
...
baseline_env=ATLAS_GEMMA4_QK_NORM_ROPE_EXPERIMENT=baseline
candidate_env=ATLAS_GEMMA4_QK_NORM_ROPE_EXPERIMENT=fused
runs=5
promotion=true
while (($#)); do
    case "$1" in
...
artifact_dir="artifacts/phase-12.3-qk-norm-rope-ab/${stamp}"
...
jq -n --arg baseline_env "$baseline_env" --arg candidate_env "$candidate_env" --argjson promotion "$promotion" \
  ...
  ' | tee "$artifact_dir/qk-norm-rope-ab-summary.json"

echo "QK norm/RoPE A/B: $artifact_dir/qk-norm-rope-ab-summary.json"
jq -e '.pass == true' "$artifact_dir/qk-norm-rope-ab-summary.json" >/dev/null || { echo "QK NORM/ROPE: NO PROMOTION" >&2; exit 1; }
[[ "$promotion" == true ]] && echo "QK NORM/ROPE: PASS" || echo "QK NORM/ROPE SCREEN: PASS"
