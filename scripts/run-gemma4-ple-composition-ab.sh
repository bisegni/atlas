#!/usr/bin/env bash
# Decode-only PLE GELU/offset composition A/B. --screen uses two runs.
set -euo pipefail

repo_root=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
cd "$repo_root"

baseline_env=ATLAS_GEMMA4_PLE_COMPOSITION_EXPERIMENT=baseline
candidate_env=ATLAS_GEMMA4_PLE_COMPOSITION_EXPERIMENT=fused
runs=5
promotion=true
while (($#)); do
    case "$1" in
...
artifact_dir="artifacts/phase-12.3-ple-composition-ab/${stamp}"
    ...
    for workload in short long; do
        ...
        for run in $(seq 1 "$runs"); do
            env "${clean_env[@]}" "$env_assignment" cargo run --release -p atlas-cli -- benchmark \
                ...
                --warmup-decode-tokens "$warmup" --decode-tokens "$measured" --max-context "$context" \
                > "$mode_dir/$workload-$run.json" 2> "$mode_dir/$workload-$run.log" || {
...
jq -n --arg baseline_env "$baseline_env" --arg candidate_env "$candidate_env" --argjson promotion "$promotion" \
  ...
  ($baseline_short[0]) as $bs | ($baseline_long[0]) as $bl | ($candidate_short[0]) as $cs | ($candidate_long[0]) as $cl |
  {baseline_environment:$baseline_env, candidate_environment:$candidate_env,
   baseline:{short:$bs,long:$bl}, candidate:{short:$cs,long:$cl}, comparison:{
    ...
    baseline_pipeline_selected: ($bs.records[0].selected_kernels.ple_composition == "gelu_f32+vector_multiply_offset_f32" and $bl.records[0].selected_kernels.ple_composition == "gelu_f32+vector_multiply_offset_f32"),
    candidate_pipeline_selected: ($cs.records[0].selected_kernels.ple_composition == "ple_gelu_multiply_offset_f32" and $cl.records[0].selected_kernels.ple_composition == "ple_gelu_multiply_offset_f32"),
    dispatches_reduced: ($cs.records[0].measured_decode_dispatch_calls < $bs.records[0].measured_decode_dispatch_calls and $cl.records[0].measured_decode_dispatch_calls < $bl.records[0].measured_decode_dispatch_calls),
    baseline_short_tok_s:$bs.median_decode_tok_s,candidate_short_tok_s:$cs.median_decode_tok_s,baseline_long_tok_s:$bl.median_decode_tok_s,candidate_long_tok_s:$cl.median_decode_tok_s,promotion_eligible:$promotion
   }}
  ...
  | .pass = (all(.baseline[]; .pass) and all(.candidate[]; .pass) and all(.comparison["exact_short","exact_long","stable_short_accounting","stable_long_accounting","baseline_pipeline_selected","candidate_pipeline_selected","dispatches_reduced","short_not_regressed"]; . == true) and (if .comparison.promotion_eligible then .comparison.long_speedup_percent >= 0 else true end))
  ' | tee "$artifact_dir/ple-composition-ab-summary.json"

echo "PLE composition A/B: $artifact_dir/ple-composition-ab-summary.json"
jq -e '.pass == true' "$artifact_dir/ple-composition-ab-summary.json" >/dev/null || { echo "PLE COMPOSITION: NO PROMOTION" >&2; exit 1; }
[[ "$promotion" == true ]] && echo "PLE COMPOSITION: PASS" || echo "PLE COMPOSITION SCREEN: PASS"
