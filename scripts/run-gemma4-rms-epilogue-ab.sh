#!/usr/bin/env bash
# Decode-only post-attention RMS + residual fusion A/B (1 dispatch/layer saved).
...
baseline_env=ATLAS_GEMMA4_RMS_EPILOGUE_EXPERIMENT=baseline
candidate_env=ATLAS_GEMMA4_RMS_EPILOGUE_EXPERIMENT=fused
runs=5
promotion=true
while (($#)); do
    case "$1" in
...
artifact_dir="artifacts/phase-12.3-rms-epilogue-ab/${stamp}"
...
jq -n --arg baseline_env "$baseline_env" --arg candidate_env "$candidate_env" --argjson promotion "$promotion" \
  ...
  ' | tee "$artifact_dir/rms-epilogue-ab-summary.json"

echo "RMS epilogue A/B: $artifact_dir/rms-epilogue-ab-summary.json"
jq -e '.pass == true' "$artifact_dir/rms-epilogue-ab-summary.json" >/dev/null || { echo "POST-ATTN RMS+RESIDUAL: NO PROMOTION" >&2; exit 1; }
[[ "$promotion" == true ]] && echo "POST-ATTN RMS+RESIDUAL: PASS" || echo "POST-ATTN RMS+RESIDUAL SCREEN: PASS"
