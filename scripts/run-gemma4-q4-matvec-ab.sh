#!/usr/bin/env bash
# Exact mixed-Q4/Q6 Q4 projection-family A/B: --screen uses two runs.
set -euo pipefail

repo_root=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
cd "$repo_root"

baseline_env=ATLAS_GEMMA4_Q4_MATVEC_EXPERIMENT=baseline
candidate_env=ATLAS_GEMMA4_Q4_MATVEC_EXPERIMENT=simdgroup_tiled
runs=5
promotion=true
while (($#)); do
    case "$1" in
...
artifact_dir="artifacts/phase-12a-q4-matvec-ab/${stamp}"
...
run_mode() {
    ...
    for workload in long short; do
        ...
        for run in $(seq 1 "$runs"); do
            if ! env -u ATLAS_GEMMA4_RMS_EPILOGUE_EXPERIMENT \
                ...
                -u ATLAS_GEMMA4_PREFILL_TOKEN_MAJOR \
        ...
        jq -s --arg workload "$workload" --argjson runs "$runs" '
          ...
          . as $records | {
            ...
            checks: {
              ...
              selected_q4_kernel: all($records[]; .selected_kernels.q4_projection != "none"),
              selected_q4_qkv_kernel: all($records[]; .selected_kernels.q4_qkv_projection != "none"),
              selected_q4_gate_up_kernel: all($records[]; .selected_kernels.q4_gate_up_projection != "none"),
              selected_q4_batch_kernel: all($records[]; .selected_kernels.q4_batch_projection != "none")
            }
...
jq -n --arg baseline_env "$baseline_env" --arg candidate_env "$candidate_env" --argjson promotion "$promotion" \
  ...
  --slurpfile candidate_short "$artifact_dir/candidate/short-summary.json" '
    ...
    | {baseline_environment: $baseline_env, candidate_environment: $candidate_env, baseline: {long: $base_long, short: $base_short}, candidate: {long: $candidate_long, short: $candidate_short}, comparison: {
        ...
        baseline_pipeline_selected: ($base_long.records[0].selected_kernels.q4_projection == "matvec_q4_0_16row" and $base_short.records[0].selected_kernels.q4_projection == "matvec_q4_0_16row" and $base_long.records[0].selected_kernels.q4_qkv_projection == "matmul_q4_0_qkv_16row" and $base_short.records[0].selected_kernels.q4_qkv_projection == "matmul_q4_0_qkv_16row" and $base_long.records[0].selected_kernels.q4_gate_up_projection == "matmul_q4_0_gate_up_16row" and $base_short.records[0].selected_kernels.q4_gate_up_projection == "matmul_q4_0_gate_up_16row" and $base_long.records[0].selected_kernels.q4_batch_projection == "matmul_q4_0_batch_16row" and $base_short.records[0].selected_kernels.q4_batch_projection == "matmul_q4_0_batch_16row"),
        candidate_pipeline_selected: ($candidate_long.records[0].selected_kernels.q4_projection == "matvec_q4_0_16row_simdgroup_tiled" and $candidate_short.records[0].selected_kernels.q4_projection == "matvec_q4_0_16row_simdgroup_tiled" and $candidate_long.records[0].selected_kernels.q4_qkv_projection == "matmul_q4_0_qkv_16row_simdgroup_tiled" and $candidate_short.records[0].selected_kernels.q4_qkv_projection == "matmul_q4_0_qkv_16row_simdgroup_tiled" and $candidate_long.records[0].selected_kernels.q4_gate_up_projection == "matmul_q4_0_gate_up_16row_simdgroup_tiled" and $candidate_short.records[0].selected_kernels.q4_gate_up_projection == "matmul_q4_0_gate_up_16row_simdgroup_tiled" and $candidate_long.records[0].selected_kernels.q4_batch_projection == "matmul_q4_0_batch_16row_simdgroup_tiled" and $candidate_short.records[0].selected_kernels.q4_batch_projection == "matmul_q4_0_batch_16row_simdgroup_tiled"),
  ...
  ' | tee "$artifact_dir/q4-matvec-ab-summary.json"

echo "Q4 matvec A/B: $artifact_dir/q4-matvec-ab-summary.json"
if jq -e '.pass == true' "$artifact_dir/q4-matvec-ab-summary.json" >/dev/null; then
    if [[ "$promotion" == true ]]; then
        echo "Q4 MATVEC A/B: PASS"
    else
        echo "Q4 MATVEC SCREEN: PASS (not eligible for promotion)"
    fi
else
    echo "Q4 MATVEC A/B: NO PROMOTION" >&2
    exit 1
fi
