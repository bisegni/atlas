#!/usr/bin/env bash
# Decode-only Q4 Gate/Up + GELU A/B. --screen uses two runs; normal uses five.
set -euo pipefail

repo_root=$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)
cd "$repo_root"

baseline_env=ATLAS_GEMMA4_FFN_GATE_UP_ACTIVATION_EXPERIMENT=baseline
candidate_env=ATLAS_GEMMA4_FFN_GATE_UP_ACTIVATION_EXPERIMENT=fused
runs=5
promotion=true
while (($#)); do
    case "$1" in
...
artifact_dir="artifacts/phase-12.3-q4-gate-up-activation-ab/${stamp}"
...
jq -n --arg baseline_env "$baseline_env" --arg candidate_env "$candidate_env" --argjson promotion "$promotion" \
  ...
  {baseline_environment:$baseline_env,candidate_environment:$candidate_env,
   baseline:{long:$bl,short:$bs},candidate:{long:$cl,short:$cs},comparison:{
    ...
    expected_resident_reduction: ($bl.records[0].resident_bytes - $cl.records[0].resident_bytes == $bl.records[0].ffn_gate_up_scratch_bytes and $bs.records[0].resident_bytes - $cs.records[0].resident_bytes == $bs.records[0].ffn_gate_up_scratch_bytes and $cl.records[0].ffn_gate_up_scratch_bytes == 0 and $cs.records[0].ffn_gate_up_scratch_bytes == 0),
    ...
    candidate_pipeline_selected: ($cl.records[0].selected_kernels.q4_gate_up_projection == "none" and $cl.records[0].selected_kernels.ffn_gate_up_activation == "matmul_q4_0_gate_up_gelu_16row"),
  ...
  | .pass = (all(.baseline[]; .pass) and all(.candidate[]; .pass) and all(.comparison["exact_long","exact_short","selected_formats_identical","stable_long_accounting","stable_short_accounting","expected_resident_reduction","baseline_pipeline_selected","candidate_pipeline_selected","dispatches_reduced","short_not_regressed"]; . == true) and (if .comparison.promotion_eligible then .comparison.long_improved else (.comparison.long_speedup_percent >= 0) end))
  ' | tee "$artifact_dir/q4-gate-up-activation-ab-summary.json"

echo "Q4 Gate/Up activation A/B: $artifact_dir/q4-gate-up-activation-ab-summary.json"
jq -e '.pass == true' "$artifact_dir/q4-gate-up-activation-ab-summary.json" >/dev/null || { echo "Q4 GATE/UP ACTIVATION: NO PROMOTION" >&2; exit 1; }
[[ "$promotion" == true ]] && echo "Q4 GATE/UP ACTIVATION: PASS" || echo "Q4 GATE/UP ACTIVATION SCREEN: PASS"
