//! Tolerance parity for the Flash16-v7 single-pass matrix-unit prefill
//! attention kernel (phase-13.14 Lever 1 follow-up).  v7 visits each key block
//! once: online-merged row max/denominator, per-block O-fragment rescaling
//! when the row max moves, unnormalized P = exp(S - M) via O += P·V, and a
//! single final denominator normalization.  It computes the same attention as
//! the v5 shared-head kernel with fp16 matrix-unit rounding and a different
//! reduction order, so parity is asserted with a relative tolerance against
//! v5.  Cases cover uniform, causal, per-token sliding-window (key_start > 0),
//! tail-tile, and zero-count masking; the v7 output buffer is sized to the
//! 8-token tile boundary because tail tiles write zero rows past the batch.
...
fn compare_rel(label: &str, reference: &[f32], candidate: &[f32]) {
    ...
    assert!(
        max_rel < 1e-2,
        "{label}: flash16-v7 diverges from v5 (max_rel={max_rel:.3e}, max_abs={max_abs:.3e})"
    );
}
...
fn run(head_dim: usize, batch: usize, controls: &[(u32, u32)], label: &str) {
    ...
    let out_v7 = runtime
        .upload_f32(&vec![0.0f32; tile_rows * q_width])
        .unwrap();
    {
        let mut command = runtime.begin_resident_command().unwrap();
        command
    ...
    compare_rel(
        ...
        &runtime.read_f32(&out_v7, batch * q_width).unwrap(),
    );
}

#[test]
fn flash16_v7_full_uniform_matches_v5_relative() {
    run(512, 16, &vec![(0, 64); 16], "full-uniform");
}

#[test]
fn flash16_v7_full_causal_matches_v5_relative() {
    let controls = (0..16)
...
#[test]
fn flash16_v7_full_tail_matches_v5_relative() {
    let controls = (0..10)
        .map(|token| (0u32, token as u32 + 1))
        .collect::<Vec<_>>();
...
#[test]
fn flash16_v7_full_zero_count_matches_v5_relative() {
    let controls = (0..16)
        .map(|token| {
            if token == 5 {
                (0u32, 0u32)
...
#[test]
fn flash16_v7_swa_uniform_matches_v5_relative() {
    run(256, 16, &vec![(0, 64); 16], "swa-uniform");
}

#[test]
fn flash16_v7_swa_window_matches_v5_relative() {
    let window = 32u32;
    let controls = (0..40)
        .map(|token| {
...
#[test]
fn flash16_v7_swa_tail_matches_v5_relative() {
    let window = 32u32;
    let controls = (0..36)
        .map(|token| {
            let position = token as u32 + 1;
